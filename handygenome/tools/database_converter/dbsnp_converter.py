...
DEFAULT_OUTFILE_PATH_GRCH37 = os.path.join(os.getcwd(), 
                                           DEFAULT_OUTFILE_BASENAME_GRCH37)
DEFAULT_OUTFILE_PATH_GRCH38 = os.path.join(os.getcwd(), 
                                           DEFAULT_OUTFILE_BASENAME_GRCH38)
...
def argument_parser(cmdargs):
    parser_dict = workflow.init_parser(
        description=(f'The original dbSNP vcf file is downloaded, then '
    ...
    parser_dict['optional'].add_argument(
        ...
        required=False, help=f'Output vcf file path for grch37')

    parser_dict['optional'].add_argument(
        '--outfile-grch38', dest='outfile_path_grch38', 
        default=DEFAULT_OUTFILE_PATH_GRCH38,
        required=False, help=f'Output vcf file path for grch38')
...
def get_freq_classes(download_path, logger):
    ...
    with pysam.VariantFile(download_path, 'r') as in_vcf:
        ...
        for vr in in_vcf.fetch():
            NR += 1
            if NR % 10_000_000 == 0:
                logger.info(f'get_freq_classes: Processing {NR:,}th variant '
                            f'record:\n{vr}')
...
def get_new_header(chromdict, freq_classes, refver, build='155'):
    ...
    new_header.add_meta(key='INFO',
                        items=[('ID', 'rs'), ('Type', 'Integer'), 
                               ('Number', 1), 
                               ('Description', 'dbSNP rs ID')])
    new_header.add_meta(key='INFO',
                        items=[('ID', 'dbSNPBuildID'), ('Type', 'Integer'), 
                               ('Number', 1), 
                               ('Description', 'First dbSNP Build for RS')])
    new_header.add_meta(
        key='INFO',
        items=[('ID', 'COMMON'), ('Type', 'Flag'), ('Number', 0), 
               ('Description', 
    ...
    for pop in freq_classes:
        new_header.add_meta(
            key='INFO',
            items=[('ID', f'AF_{pop}'), ('Type', 'Float'), ('Number', 1), 
                   ('Description', 
                    f'Allele frequency in the population \'{pop}\'')])
...
def handle_freq(freq):
    ...
    for item in freq_split:
        ...
        freq_dict[item_split[0]] = [ 
            (float(0) if x == '.' else float(x))
            for x in item_split[1].split(',')]

    return freq_dict


def get_new_vr_list(vr, chromdict, chrname_converter, new_header, 
                    freq_classes, infile_vcfspecs):
    new_vr_list = list()
    infile_vcfspecs.setdefault(vr.contig, set())

    new_chrom = chrname_converter[vr.contig]
    if (
            (new_chrom is not None) and
            (new_chrom in chromdict.contigs) and
            'FREQ' in vr.info):
        freq = vr.info['FREQ']
...
def write_outfile(download_path, outfile_path, chromdict, chrname_converter, 
                  new_header, freq_classes, logger):
    infile_vcfspecs = dict()
    with \
            pysam.VariantFile(outfile_path, mode='wz', 
                              header=new_header) as out_vcf, \
            pysam.VariantFile(download_path, mode='r') as in_vcf:
        NR = 0
        for vr in in_vcf.fetch():
            NR += 1
            if NR % 1_000_000 == 0:
                logger.info(f'write_outfile: Processing {NR:,}th variant '
                            f'record:\n{vr}')

            new_vr_list = get_new_vr_list(vr, chromdict, chrname_converter, 
                                          new_header, freq_classes, 
                                          infile_vcfspecs)
            for new_vr in new_vr_list:
                out_vcf.write(new_vr)


def submain(download_path, download_url, outfile_path, chromdict, 
            chrname_converter, refver, build, logger):
    # download
    ...
    # write output file
    new_header = get_new_header(chromdict, freq_classes, refver=refver,
                                build=build)
    logger.info('Modifying and writing original variant records')
    write_outfile(download_path, outfile_path, chromdict, chrname_converter, 
                  new_header, freq_classes, logger)
...
def submain_grch38(args, logger):
    ...
    submain(download_path=args.download_path_grch38, 
