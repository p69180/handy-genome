...
DEFAULT_OUTFILE_PATH_GRCH37 = os.path.join(os.getcwd(), re.sub('gff.gz$', 'grch37.gff.gz', DEFAULT_OUTFILE_BASENAME))
DEFAULT_OUTFILE_PATH_GRCH38 = os.path.join(os.getcwd(), re.sub('gff.gz$', 'grch38.gff.gz', DEFAULT_OUTFILE_BASENAME))
	...
	with open(outfile_path_uncomp, 'wt') as outfile:
		for line_data in mainfile_data_sorted:
			...
			line_data['attrs']['activity'] = ','.join(
				f'{tissue}:{activity.setdefault(tissue, "NA")}' for tissue in tissue_list
			)
			attrs_string = ';'.join(f'{key}={val}' for key, val in line_data['attrs'].items())
			line_data['linesp'].append(attrs_string)
			outfile.write('\t'.join(line_data['linesp']) + '\n')


def convert_common(logger, mainfile_url, ftp_activity_directory, refver, output_chrname_version, outfile_path, download_dir):
	# download
	if download_dir is None:
		download_dir = common.get_tmpfile_path(delete = False, is_dir = True)
		mainfile_path = os.path.join(download_dir, 'main.gff.gz')
		activity_dir = os.path.join(download_dir, 'activity')
		os.mkdir(activity_dir)
		download(logger, mainfile_url, ftp_activity_directory, download_dir, mainfile_path, activity_dir)
