...
def argument_parser(cmdargs):
	parser_dict = workflow.init_parser(
		description = f'The original gff3 gene set files are downloaded, filtered, sorted, bgzipped, and tabix-indexed. These lines are filtered out: containing "###" or "feature" value is "chromosome" or "supercontig".')
	...
	parser_dict['optional'].add_argument(
		...
		required = False, 
		help = f'(Optional) previously downloaded original gene set gff3 file path for grch37')
...
def get_line_lists(download_path, chromdict, assembly_spec, output_chrname_version):
...
def write_uncompressed_file(outfile_path_uncomp, line_list_comment, linesp_list_sorted): 
...
def convert_grch37(logger, outfile_path, download_path = None):
	convert_common(URL_GRCH37, 'nochr_plus_genbank', 'hg19', logger, outfile_path, download_path)


def convert_grch38(logger, outfile_path, download_path = None):
	convert_common(URL_GRCH38, 'ucsc', 'hg38', logger, outfile_path, download_path)


def main(cmdargs):
	args = argument_parser(cmdargs)
	logger = workflow.get_logger(name = 'gff_converter')
