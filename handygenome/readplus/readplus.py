...
class ReadPlus:
    ...
    def get_pairs_indexes(self, range0):
        ...
        if not (self.read.reference_start <= range0.start and 
                self.read.reference_end >= range0.stop):
...
def get_rpplist_sv(bam, fasta, chromdict, bnds, view=False,
    ...
    if relevant_qname_list_union is None:
        ...
        rpplist_sortkey = lambda rpp: min(rpp.rp1.read.reference_start,
                                          rpp.rp2.read.reference_start)
        rpplist.sort(key=rpplist_sortkey)
...
def get_initial_fetcher_sv(bam, bnds, fetch_padding_sv, fetch_padding_common):
    ...
    fetcher_bnd1 = subfun(bam, bnds.chrom_bnd1, bnds.get_pos_range0_bnd1(),
                          bnds.endis5_bnd1,
                          fetch_padding_common, fetch_padding_sv)
    fetcher_bnd2 = subfun(bam, bnds.chrom_bnd2, bnds.get_pos_range0_bnd2(),
                          bnds.endis5_bnd2,
                          fetch_padding_common, fetch_padding_sv)
...
def get_initial_fetcher_sv_view(bam, bnds, fetch_padding_view):
    fetcher_bnd1 = readhandler.get_fetch(
        bam, bnds.chrom_bnd1,
        start=(min(bnds.get_pos_range0_bnd1()) - fetch_padding_view),
        end=(max(bnds.get_pos_range0_bnd1()) + 1 + fetch_padding_view),
        filter_fun=readhandler.check_bad_read)

    fetcher_bnd2 = readhandler.get_fetch(
        bam, bnds.chrom_bnd2,
        start=(min(bnds.get_pos_range0_bnd2()) - fetch_padding_view),
        end=(max(bnds.get_pos_range0_bnd2()) + 1 + fetch_padding_view),
        filter_fun=readhandler.check_bad_read)
...
def initial_fetch_sv(bam, bnds, view, fetch_padding_common,
    ...
    fetcher_routine(fetcher_bnd1, view, bnds.get_pos_range0_bnd1(), 
                    bnds.endis5_bnd1, relevant_qname_list_bnd1, 
                    start0_list_bnd1, long_insert_threshold)
    fetcher_routine(fetcher_bnd2, view, bnds.get_pos_range0_bnd2(),
...
def refined_fetch_sv(bam, bnds, new_fetch_range_bnd1, new_fetch_range_bnd2,
    ...
    if new_fetch_range_bnd1 is not None:
        fetch = readhandler.get_fetch(bam, bnds.chrom_bnd1,
    ...
    if new_fetch_range_bnd2 is not None:
        fetch = readhandler.get_fetch(bam, bnds.chrom_bnd2,
