...
CONSEQUENCE_TRANSCRIPT_VALUES = set( itertools.chain.from_iterable( CONSEQUENCE_TRANSCRIPT_CLASSES.values() ) )
...
BIOTYPE_TRANSCRIPT_CLASSES = {
	...
	'noncoding': (
			...
			'Y_RNA',
			'antisense_RNA',
			'antisense', # updated on 220316 ; biotype is 'antisense' in ensembl rest vep result for ENST00000442411
			'lncRNA',
			'lincRNA',
			'miRNA',
			'misc_RNA',
...
def set_transcript_subtypes(annotitem, biotype, feature_id):
	...
	if biotype not in BIOTYPE_TRANSCRIPT_VALUES:
		raise Exception(f'Unexpected biotype value; biotype: {biotype} ; feature_id: {feature_id}')
...
def set_regulatory_subtypes(annotitem, raw_regulatory_type, option):
	...
	Args:
		annotitem: A dictionary
		raw_regulatory_type: Raw regulatory feature type string obtained from vep, rest, etc.
	"""

	assert option in REGULATORY_FEATURE_TYPES.keys()
	assert raw_regulatory_type in REGULATORY_FEATURE_TYPES[option].keys(), f'Unexpected regulatory type "{raw_regulatory_type}", option "{option}"'
	...
	for val in REGULATORY_FEATURE_TYPES[option].values():
		annotitem['regulatory_subtype_flags'][f'is_{val}'] = ( REGULATORY_FEATURE_TYPES[option][ raw_regulatory_type ] == val )
...
def set_consequence_attributes(annotitem, consequences, feature_id):
	...
	if not consequences_set.issubset(CONSEQUENCE_TRANSCRIPT_VALUES):
		unexpected_consequences = consequences_set.difference(CONSEQUENCE_TRANSCRIPT_VALUES)
		raise Exception(f'Unexpected consequence value; consequence: {unexpected_consequences} ; feature_id: {feature_id}')

	annotitem['consequence_flags'] = dict()
	for key, val in CONSEQUENCE_TRANSCRIPT_CLASSES.items():
		annotitem['consequence_flags'][key] = ( len(consequences_set.intersection(val)) != 0 )


def set_distance(annotitem, consequences, distance_value):
	assert 'is_forward' in annotitem
	assert distance_value is not None
	assert len( {'upstream_gene_variant', 'downstream_gene_variant'}.intersection(consequences) ) == 1
		...
		if len(codon_change[0]) == len(codon_change[1]):
			if len(codon_change[0]) == 3:
				annotitem['codon_frame0'] = [
					x[0] for x in enumerate(codon_change[0]) if x[1].isupper()
				][0]
			else:
				raise Exception(f'A feature with non-length-3 "codon_change" value:\n{annotitem}')
		else:
			annotitem['codon_frame0'] = None
...
def extract_cmdline_vep_annotation(vr):
	if infoformat.check_NA_info(vr, veplib.VEP_INFO_FIELD):
		...
		for item in vr.info[veplib.VEP_INFO_FIELD]:
			raw_result_item = dict(zip(
				vepkeys, 
				( 
					( None if x == '' else common.str_to_nonstr(x) )
				 	for x in item.split('|') 
				),
			))
...
def parse_cmdline_vep(vr):
		...
		def parse_clinvar(item, result):
			if 'VAR_SYNONYMS' in item:
				...
				for subitem in item['VAR_SYNONYMS'].split('--'):
					...
					var_synonyms[ subitem_split[0] ] = subitem_split[1].split('&')
	...
	def subfun_featuretype(annotitem, raw_result_item):
		assert raw_result_item['Feature_type'] in ('Transcript', 'RegulatoryFeature', 'MotifFeature'), f'Unexpected "Feature_type":\n{raw_result_item}'
	...
	def subfun_set_polyphen_sift(annotitem, raw_result_item):
		for option in ('polyphen', 'sift'):
			raw_value = raw_result_item['PolyPhen'] if (option == 'polyphen') else raw_result_item['SIFT']
	...
	def subfun_transcript(annotitem, raw_result_item):
		...
		annotitem['aa_change'] = None if (raw_result_item['Amino_acids'] is None) else raw_result_item['Amino_acids'].split('/')
		annotitem['codon_change'] = None if (raw_result_item['Codons'] is None) else raw_result_item['Codons'].split('/')
		...
		set_transcript_subtypes(annotitem, raw_result_item['BIOTYPE'], raw_result_item['Feature'])
		set_is_forward(annotitem, raw_result_item['STRAND'])
		set_consequence_attributes(annotitem, annotitem['consequences'], raw_result_item['Feature'])
		subfun_set_distance(annotitem, raw_result_item, annotitem['consequences'])
...
def _parse_rest_lookup_transcript_annotitem(raw_result, refver, set_gene_name = True):
	...
	try:
		...
		raise Exception(f'Unexpected rest lookup transcript ID result:\n{raw_result}') from e

	annotitem['gene_id'] = raw_result['Parent']
	if set_gene_name:
		try:
			gene_name = ensembl_rest.lookup_id(annotitem['gene_id'], refver = refver, expand = False)['display_name']
		except KeyError as e:
			raise Exception(f'Unexpected rest lookup gene ID result:\n{raw_result}') from e
...
def parse_rest_lookup_transcript(raw_result, refver, set_gene_name = True):
	annotitem = _parse_rest_lookup_transcript_annotitem(raw_result, refver = refver, set_gene_name = set_gene_name)
...
def parse_rest_lookup_transcript_post(raw_result, refver, set_gene_name = True):
	...
	for val in raw_result.values():
		annotitem = _parse_rest_lookup_transcript_annotitem(val, refver = refver, set_gene_name = set_gene_name)
		transcript[annotitem['id']] = annotitem
...
def parse_rest_regulatory(raw_result):
	...
	set_regulatory_subtypes(annotitem, raw_result['feature_type'], 'regulatory')
...
def parse_rest_overlap(raw_result, refver, include_motif_without_evidence = False):
	...
	def subfun_transcript(annotitem, raw_result_item, refver):
		annotitem['biotype'] = raw_result_item['biotype']
		set_transcript_subtypes(annotitem, raw_result_item['biotype'], raw_result_item['id'])
		...
		annotitem['gene_name'] = ensembl_rest.lookup_id(annotitem['gene_id'], refver = refver, expand = False)['display_name']
	...
	def subfun_regulatory(annotitem, raw_result_item):
		...
		set_regulatory_subtypes(annotitem, raw_result_item['description'], 'overlap')

	def subfun_motif(annotitem, raw_result_item):
		if 'epigenomes_with_experimental_evidence' in raw_result_item:
			annotitem['is_with_evidence'] = True
			annotitem['evidence_name'] = '&'.join( raw_result_item['epigenomes_with_experimental_evidence'].split(',') )
		else:
		...
		annotitem['TF'] = ','.split( raw_result_item['transcription_factor_complex'] )

	def subfun_repeat(annotitem, raw_result_item):
		annotitem['repeat_type'] = raw_result_item['description']
		annotitem['id'] = '_'.join((annotitem['chrom'], str(annotitem['start0']), 
	...
	def main(raw_result, refver, include_motif_without_evidence):
		...
		for raw_result_item in raw_result:
			assert raw_result_item['feature_type'] in ('transcript', 'regulatory', 'motif', 'repeat')
			annotitem = annotationdb.AnnotItem()
...
def parse_rest_vep(raw_result):
	...
	def subfun_transcript(dic):
		...
		annotitem['gene_name'] = dic['gene_symbol'] if ('gene_symbol' in dic) else None # 'TEC' biotype transcript does not have a gene symbol
		annotitem['is_canonical'] = ('canonical' in dic)
		annotitem['mane_select'] = dic['mane_select'] if ('mane_select' in dic) else None

		annotitem['ccds_id'] = dic['ccds'] if ('ccds' in dic) else None
		annotitem['protein_id'] = dic['protein_id'] if ('protein_id' in dic) else None
		annotitem['aa_change'] = dic['amino_acids'].split('/') if ('amino_acids' in dic) else None
		annotitem['codon_change'] = dic['codons'].split('/') if ('codons' in dic) else None
		set_codon_frame0(annotitem)

		annotitem['variant_start0_transcript'] = dic['cdna_start'] - 1 if ('cdna_start' in dic) else None
		annotitem['variant_start1_transcript'] = dic['cdna_start'] if ('cdna_start' in dic) else None
		annotitem['variant_end0_transcript'] = dic['cdna_end'] if ('cdna_end' in dic) else None
		annotitem['variant_end1_transcript'] = dic['cdna_end'] if ('cdna_end' in dic) else None

		annotitem['variant_start0_cds'] = dic['cds_start'] - 1 if ('cds_start' in dic) else None
		annotitem['variant_start1_cds'] = dic['cds_start'] if ('cds_start' in dic) else None
		annotitem['variant_end0_cds'] = dic['cds_end'] if ('cds_end' in dic) else None
		annotitem['variant_end1_cds'] = dic['cds_end'] if ('cds_end' in dic) else None

		annotitem['variant_start0_protein'] = dic['protein_start'] - 1 if ('protein_start' in dic) else None
		annotitem['variant_start1_protein'] = dic['protein_start'] if ('protein_start' in dic) else None
		annotitem['variant_end0_protein'] = dic['protein_end'] if ('protein_end' in dic) else None
		annotitem['variant_end1_protein'] = dic['protein_end'] if ('protein_end' in dic) else None
		...
		set_transcript_subtypes(annotitem, dic['biotype'], dic['transcript_id'])
		set_is_forward(annotitem, dic['strand'])
		set_consequence_attributes(annotitem, annotitem['consequences'], dic['transcript_id'])
