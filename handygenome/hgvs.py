...
def pattern_matcher(hgvsg):
	...
	if len(matched_patterns) == 1:
		...
		raise Exception(f'More than one pattern match for this hgvsg string: {hgvsg}')


def modify_chrom(chrom, fasta):
	"""
	When input 'chrom' is not included in the reference contig names, (give by 'fasta')
	guesses an appropriate contig name and returns it.
	For example, if 'fasta' contig names include 'chr1', 'chr2', ... but input 'chrom' is '4', returns 'chr4'.
	"""
	...
	if chrom in fasta.references:
		...
		if chrom_mat is not None:
			if 'chr1' in fasta.references:
				...
				raise Exception(f'Input fasta contig names do not include "1" nor "chr1".')
			...
			if 'chrM' in fasta.references:
				...
				raise Exception(f'Input "chrom" is mitochondrial but input fasta object does not include a mitonchondrial contig.')

		else:
			raise Exception(f'Input "chrom" ({chrom}) does not fit to the pattern of numbered chromosomes or mitochondria.')
...
def hgvsg_to_vcfspec(hgvsg, fasta, leftmost = True):
	def sanitycheck_pos1_lt_pos2(mat, hgvsg):
		if mat.group('pos2') is not None:
			if int(mat.group('pos1')) >= int(mat.group('pos2')):
				raise Exception(f'pos1 is not less than pos2 in the input hgvsg string: {hgvsg}')
	...
	if hgvs_mttype == 'snv':
		...
		fetch_end = pos + 1 if (mat.group('pos2') is None) else int(mat.group('pos2'))
		...
		if abs( int(mat.group('pos1')) - int(mat.group('pos2')) ) != 1: 
			raise Exception(f'Input hgvsg pattern is assumed to be insertion but difference between pos1 and pos2 is not 1 : {hgvsg}')
		
		pos = min( int(mat.group('pos1')), int(mat.group('pos2')) )
			# In COSMIC v95 GRCh38 CosmicNCV.tsv.gz file, HGVSG for COSV58999368 is 18:g.12817321_12817320insA
	...
	# when ref or alt contains 'n' or 'N', functions below do not effectively work
	if leftmost:
...
def hgvsg_to_vcfspec_refver(hgvsg, refver):
	...
	return hgvsg_to_vcfspec_fastapath(hgvsg, common.DEFAULT_FASTA_PATH_DICT[refver])
...
def hgvsc_to_hgvsg(hgvsc, hg19):
	...
	if hg19:
		raw_result = ensembl_rest.get_url_contents(f'http://grch37.rest.ensembl.org/variant_recoder/human/{hgvsc}')
	else:
		raw_result = ensembl_rest.get_url_contents(f'http://rest.ensembl.org/variant_recoder/human/{hgvsc}')

	if len(raw_result) != 1:
		raise Exception(f'Unexpected variantrecoder raw result pattern: {raw_result}')
	...
	hgvsg_results_filtered = [ x for x in hgvsg_results if not x.startswith('LRG') ]
	if len(hgvsg_results_filtered) != 1:
		raise Exception(f'Unexpected variantrecoder hgvsg results pattern: {hgvsg_results}')
	...
	namemap = chrnames.SPECS['grch37'] if hg19 else chrnames.SPECS['grch38']
	if raw_contig not in namemap.data['refseq']:
		raise Exception(f'contig name of variantrecoder result is not refseq: {raw_hgvsg}')
