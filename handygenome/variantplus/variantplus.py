...
class VariantPlus:
    ...
    def load_readstats(self, sampleid):
    ...
    def get_ponfilter(self, sampleids=None, **kwargs):
        ...
        return vpfilter.PonFilter(readstats_dict, **kwargs)
    ...
    def get_vaf(self, sampleid, alleleindex=1, ndigits=3):
        readstats = self.readstats_dict[sampleid]
        alt_rppcount = readstats['rppcounts'][alleleindex]
        total_rppcount = self.get_total_rppcount(sampleid)

        if total_rppcount == 0:
            vaf = np.nan
        else: 
            vaf = round(alt_rppcount / total_rppcount, ndigits)
