...
def indel_equivalents(vcfspec, fasta, 
                      fetch_extend_length=DEFAULT_FETCH_EXTEND_LENGTH):
    """Args:
        vcfspec: Must be monoallelic.
    ...
    def progressive_fetch(chrom, pos, fasta, fetch_extend_length, diff_seq, 
                          is_ins, forward):
        extend_count = 0
        while True:
            extend_count += 1
            if forward: # for lookforward
                if is_ins:
                    fetch_start = (pos 
                                   + ((extend_count-1) * fetch_extend_length))
                    fetch_end = pos + (extend_count * fetch_extend_length)
                else: # deletion
                    fetch_start = (pos + len(diff_seq) 
                                   + ((extend_count-1) * fetch_extend_length))
                    fetch_end = (pos + len(diff_seq) 
                                 + (extend_count * fetch_extend_length))
                ...
                result = reversed(
                    list(fasta.fetch(chrom, fetch_start, fetch_end)))

            yield result

    def navigate(chrom, pos, fasta, diff_seq, new_pos_list, 
                 fetch_extend_length, is_ins, forward):
        """
        Args:
        ...
        fetched_seq = list()
        fetcher = progressive_fetch(chrom, pos, fasta, fetch_extend_length, 
                                    diff_seq, is_ins, forward)
            ...
            if fetched_seq[-1] != diff_seq_compared[idx_remainder]:
                ...
                if is_ins:
                    if forward:
                        ...
                        inserted_seq = (diff_seq_compared[idx_remainder+1:] 
                                        + diff_seq_compared[:idx_remainder+1])
                    else:
                        new_pos = pos - (idx + 1)
                        new_ref = fetch_reservoir[idx+1]
                        inserted_seq = list(
                            reversed(diff_seq_compared[idx_remainder+1:] 
                                     + diff_seq_compared[:idx_remainder+1]))
                    ...
                    if idx < len(diff_seq):
                        if forward:
                            new_pos = pos + (idx + 1)
                            deleted_seq = (diff_seq_compared[idx+1:] 
                                           + fetched_seq)
                            new_ref = diff_seq[idx] + ''.join(deleted_seq)
                        else:
                            new_pos = pos - (idx + 1)
                            deleted_seq = list(
                                reversed(diff_seq_compared[idx+1:] 
                                         + fetched_seq))
                            new_ref = (fetch_reservoir[idx+1] 
                                       + ''.join(deleted_seq))
                    ...
                    beyond_seq = (
                        fetch_reservoir[(idx + 1) : (idx + 1 + len(diff_seq))])
                    if (
                            (diff_seq_compared[idx_remainder+1:] 
                             == beyond_seq[:-(idx_remainder+1)]) and
                            (diff_seq_compared[:idx_remainder+1] 
                             == beyond_seq[-(idx_remainder+1):])):
                        if forward:
                            ...
                            new_ref = (
                                fetch_reservoir[idx + len(diff_seq) + 1] 
                                + ''.join(deleted_seq))

                        new_pos_list.append((new_pos, new_ref))

    def get_new_pos_list(chrom, pos, fasta, diff_seq, fetch_extend_length, 
                         is_ins):
        new_pos_list = list()
        navigate(chrom, pos, fasta, diff_seq, new_pos_list, 
                 fetch_extend_length, is_ins, forward=True)
        navigate(chrom, pos, fasta, diff_seq, new_pos_list, 
                 fetch_extend_length, is_ins, forward=False)

        return new_pos_list

    def get_result(chrom, pos, ref, alt, fasta, is_ins, new_pos_list, 
                   diff_seq):
        result = list()
        ...
        if is_ins:
            for new_pos, new_ref, inserted_seq in new_pos_list:
                ...
                result.append(
                    common.Vcfspec(chrom, new_pos, new_ref, [new_alt]))
        else: # deletion
    ...
    def main(vcfspec, fasta, fetch_extend_length):
        ...
        if set('nN').isdisjoint(ref) and set('nN').isdisjoint(alt):
            ...
            if mttype in ('ins', 'del'):
                ...
                new_pos_list = get_new_pos_list(
                    chrom, pos, fasta, diff_seq, fetch_extend_length, is_ins)
                result = get_result(
                    chrom, pos, ref, alt, fasta, is_ins, new_pos_list, 
                    diff_seq)
            else:
                result = [vcfspec]
        else:
