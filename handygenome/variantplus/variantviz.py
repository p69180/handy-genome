...
def show_readcounts(vp, allele_idx=1, sampleid_order=None, title=None,
    ...
    # prepare data
    altcnts = np.array([
        vp.readstats_dict[sampleid]['rppcounts'][allele_idx]
        for sampleid in sampleid_order])
    totalcnts = np.array([
        vp.get_total_rppcount(sampleid)
        for sampleid in sampleid_order])
    vaf = altcnts / totalcnts
    
    if mask is None:
        mask = np.repeat(True, len(sampleid_order))
    ...
    ax1.bar(x[mask], y1[mask], color=col_good_bar, alpha=0.7)
    ax1.bar(x[not_mask], y1[not_mask], color=col_bad_bar, alpha=0.7)
